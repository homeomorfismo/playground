...
f.Assemble()
#TODO Do I need this? RHS can be constructed as A*exact_sol
...
exact_ux = -2*x**2*y*z*(x - 1)**2*(y - 1)*(y - z)*(z - 1)*(2*y*z - y - z + 1)
exact_uy = 2*x*y**2*z*(x - 1)*(x - z)*(y - 1)**2*(z - 1)*(2*x*z - x - z + 1)
exact_uz = -2*x*y*z**2*(x - 1)*(x - y)*(y - 1)*(z - 1)**2*(2*x*y - x - y + 1)
